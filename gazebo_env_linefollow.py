...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def __init__(self):
        ...
        LAUNCH_FILE = '/home/joel/enph353_gym-gazebo-noetic/gym_gazebo/envs/ros_ws/src/linefollow_ros/launch/linefollow_world.launch'
    ...
    def process_image(self, data):
        ...
        # split image into 8 bins
        height, width, channels = cv_image.shape
        bin_width = width / 8

        # define range of blue color in RGB
        # lower_blue = np.array([60, 110, 150])
        # upper_blue = np.array([75, 130, 175])
        lower_blue = 150
        upper_blue = 170
        max_bin = 0;
        max_bin_val = 0;
        for i in range(8):
            #draw a rectangle bounding each bin
            # crop image
            crop_img = cv_image[(int)(height/2):height, int(i * bin_width):int((i + 1) * bin_width)]

            # threshold for blue in crop_img
            mask = (crop_img[:, :, 0] > lower_blue) & (crop_img[:, :, 0] < upper_blue)

            mask = mask.astype(np.uint8)
            # count white pixels
            white_pixels = np.sum(mask)
            #print("white pixels: ", white_pixels)
            if ((white_pixels > max_bin_val) & (white_pixels > 1000)):
                max_bin_val = white_pixels
                max_bin = i
                self.timeout = 1
                cv2.circle(cv_image, (int((i + 0.5) * bin_width), int(height / 2)), 5, (0, 0, 255), -1)

            else:
                state[i] = 0
            
            cv2.rectangle(cv_image, (int(i * bin_width), 0), (int((i + 1) * bin_width), height), (0, 255, 0), 2)
        ...
        mask = ((cv_image[:, :, 0] > lower_blue) & (cv_image[:, :, 0] < upper_blue))
        mask = mask.astype(np.uint8)
        ...
        # print the state onto the image 
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(cv_image, str(state), (10, 50), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
